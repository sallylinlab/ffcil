...
def set_up_datasets(args):
    ...
    if args.dataset == 'mini_imagenet':
        ...
        args.sessions = 9
    if args.dataset == 'auo':
        import dataloader.auo.auo as Dataset
        args.base_class = 15
        args.num_classes = 27
        args.way = 3
        args.shot = 5
    ...
    if args.dataset == 'stanford_dogs':
        import dataloader.stanforddogs.stanforddogs as Dataset
        args.base_class = 60    
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'auo':
        trainset = args.Dataset.AUO(root=args.dataroot, train=True,
                                             index=class_index, base_sess=True)
        testset = args.Dataset.AUO(root=args.dataroot, train=False, index=class_index)
        if args.extra_class and args.project == 'base':
            extraset = args.Dataset.AUO(root=args.dataroot, train=False, index=class_index, extra=True)
            extraset.targets = (np.array(extraset.targets)+args.base_class).tolist()
            trainset.data.extend(extraset.data)
            trainset.targets.extend(extraset.targets)

    if args.dataset == 'stanford_dogs':
        trainset = args.Dataset.StanfordDogs(root=args.dataroot, train=True,
                                       index=class_index, base_sess=True)
        testset = args.Dataset.StanfordDogs(root=args.dataroot, train=False, index=class_index)
...
def get_base_dataloader_meta(args):
    ...
    if args.dataset == 'auo':
        trainset = args.Dataset.AUO(root=args.dataroot, train=True,
                                             index_path=txt_path)
        testset = args.Dataset.AUO(root=args.dataroot, train=False,
                                            index=class_index)

    if args.dataset == 'stanford_dogs':
        trainset = args.Dataset.StanfordDogs(root=args.dataroot, train=True,
                                       index_path=txt_path)
        testset = args.Dataset.StanfordDogs(root=args.dataroot, train=False,
                                      index=class_index)
