...
class FSCILTrainer(Trainer):
    ...
    def res_train(self, model, trainloader, low_trainloader, optimizer, scheduler, epoch, args):
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            # total_loss = F.cross_entropy(logits, true_label) + self.center_loss(query.view(query_shot, query.shape[-1]), true_label)*0.1 +  self.ring_loss(query.view(query_shot, query.shape[-1]))
            # total_loss = F.cross_entropy(logits, true_label) + self.ring_loss(query.view(query_shot, query.shape[-1]))
            # total_loss = F.cross_entropy(logits, true_label) + self.ring_loss(protos.view(args.base_class+args.low_way, protos.shape[-1]))
            ...
            #     # 0.1 is center loss weight for total loss, e.g. 0.1*center_loss
