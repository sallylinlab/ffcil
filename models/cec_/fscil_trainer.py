...
class FSCILTrainer(Trainer):
    ...
    def replace_to_rotate(self, proto_tmp, query_tmp):
        ...
        # CutMix
        # pt = proto_tmp.clone().detach()
        # qt = query_tmp.clone().detach()
        # diff = False
        # while not diff:
        #     rand_index = torch.randperm(self.args.low_way).cuda()
        ...
        #     bbx1, bby1, bbx2, bby2 = self.rand_bbox(proto_tmp.shape[2:], lam)
        #     proto_tmp[i::self.args.low_way][:, :, bbx1:bbx2, bby1:bby2] = pt[rand_index[i]::self.args.low_way][:, :, bbx1:bbx2, bby1:bby2]
        #     query_tmp[i::self.args.low_way][:, :, bbx1:bbx2, bby1:bby2] = qt[rand_index[i]::self.args.low_way][:, :, bbx1:bbx2, bby1:bby2]
    ...
    def get_optimizer_res(self):
        if self.args.epochs_proj>0:
            optimizer = torch.optim.SGD([{'params': self.model.module.encoder.parameters(), 'lr': self.args.lr_base},
                                        {'params': self.model.module.projection.parameters(), 'lr': self.args.lrg}],
                                        momentum=0.9, nesterov=True, weight_decay=self.args.decay)
        else:
            optimizer = torch.optim.SGD([{'params': self.model.module.encoder.parameters(), 'lr': self.args.lr_base},
                                        {'params': self.model.module.slf_attn.parameters(), 'lr': self.args.lrg}],
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                # proj training
                print('start proj train')
                optimizer, scheduler = self.get_optimizer_res()
                result_list.append('\nproj training log')

                for epoch in range(args.epochs_proj):
                    start_time = time.time()

                    self.model.module.encoder.eval()
                    self.model.module.projection.train()
                    tl, ta = self.proj_train(self.model, trainloader, lowloader, optimizer, scheduler, epoch, args)
                    self.model.module.projection.eval()
                    self.model.eval()
                    self.model = replace_base_fc(train_set, testloader.dataset.transform, self.model, args)
                    self.model.module.mode = 'avg_cos'
                    ...
                    result_list.append(
                        'epoch:%03d,lr:%.5f,training_loss:%.5f,training_acc:%.5f,val_loss:%.5f,val_acc:%.5f' % (
                            epoch, lrc, tl, ta, vl, va))
                    print('still need around %.2f mins to finish' % (
                                  (time.time() - start_time) * (args.epochs_proj - epoch) / 60))
                    scheduler.step()
                ...
                self.model.eval()
                if args.epochs_proj>0:
                    for epoch in range(20):
                        self.model.module.projection.train()
                        trainloader.dataset.transform = testloader.dataset.transform
                        self.model.module.update_fc(trainloader, np.unique(train_set.targets), session)

                        tl, ta = self.proj_train_inc(self.model, trainloader, optimizer, scheduler, epoch, args)
                else:
                    trainloader.dataset.transform = testloader.dataset.transform
                    self.model.module.update_fc(trainloader, np.unique(train_set.targets), session)
    ...
    def res_train(self, model, trainloader, low_trainloader, optimizer, scheduler, epoch, args):
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            query_shot = (args.episode_way+args.low_way)*args.episode_query
    ...
    def proj_train(self, model, trainloader, low_trainloader, optimizer, scheduler, epoch, args):
        tl = Averager()
        ta = Averager()
        tqdm_gen = tqdm(trainloader)
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            project_protos = model.module.encode(protos, project=True)
            project_query = model.module.encode(query, project=True)

            logits = model.module.forward_metric_(project_protos, project_query)
            total_loss = F.cross_entropy(logits, true_label)
    ...
    def proj_train_inc(self, model, trainloader, optimizer, scheduler, epoch, args):
        tl = Averager()
        ta = Averager()
        tqdm_gen = tqdm(trainloader)
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            query = model(data)
            _, query_tmp = self.get_rotate_pseudo_classes(query, query)
            labels = []
    ...
    def test(self, model, testloader, args, session, getc_acc=False, new_stage=False):
        ...
        with torch.no_grad():
            for i, batch in enumerate(testloader, 1):
                ...
                if self.args.epochs_proj > 0:
                    project_protos = model.module.encode(proto, project=True)
                    project_query = model.module.encode(query, project=True)
